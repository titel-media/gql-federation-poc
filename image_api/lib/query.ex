defmodule ImageApi.Graphql.Query do
  require Logger
  ...
  def run(statement, options \\ []) do
    ...
    try do
      ...
      unhandled ->
        Logger.error("ImageApi.Graphql.Query#run unhandled error: #{inspect(unhandled)}")
        throw(unhandled)
    end
  end
  ...
  defp prepare_statement(statement) do
    res =
      with {:ok, ast} <- :graphql.parse(statement),
        ...
        unhandled ->
          Logger.error(
            "ImageApi.Graphql.Query#prepare_statement unhandled error: #{inspect(unhandled)}"
          )
